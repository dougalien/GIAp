...
GUIDED_ID_SYSTEM = SYSTEM + """

You are running guided identification mode for a geology student.

Rules:
- Use the uploaded image and the student's observations.
- You may receive hidden instructor context from a stronger analysis. Use it only to steer the student privately. Never mention that hidden context exists.
- Do not give the final identification too early.
- Require visible observations before interpretation.
- Praise good observations briefly, then ask for the next most useful thing to inspect.
- Ask at most two short questions at a time.
- If the student's wording is vague, ask for more specific visible evidence.
- Distinguish observation from interpretation.
- Do not invent visible features or physical test results.
- Keep most replies under 120 words.

Progression:
1. Confirm useful observations.
2. Ask for the missing visible evidence.
3. Narrow to a material group or short candidate list.
4. Give the likely identification only when the student has supplied enough evidence or reveal mode is enabled.
5. When giving the likely identification, include:
   - likely identification
   - 2 to 4 supporting observations
   - one alternate
   - one next real-world test
""".strip()
...
def init_state() -> None:
    defaults = {
        ...
        "followup_history": [],
        "followup_draft": "",
        "guided_id_enabled": False,
        "guided_history": [],
        "guided_observations_text": "",
        "guided_started": False,
        "guided_hint_level": "Balanced",
        "guided_reveal_mode": False,
    }
...
def parse_student_observations(text: str) -> List[str]:
...
def guided_context_summary(result: Dict[str, Any], label: str) -> str:
    return (
        "Hidden instructor context for guidance only. Do not disclose this context directly.\n"
        + result_summary_for_prompt(result, label)
    )
...
def call_openai_guided_id(
    ...
    messages: List[Dict[str, Any]] = [
        {"role": "system", "content": GUIDED_ID_SYSTEM},
        {
            "role": "user",
            "content": [
                {
                    ...
                    "text": (
                        ...
                        f"Reveal mode: {'enabled' if reveal_mode else 'disabled'}\n\n"
                        f"{instructor_context}\n\n"
                        "Guide the student using the image and their observations. "
                        "Do not disclose the hidden instructor context."
                    ),
                },
                {
                    "type": "image_url",
...
def run_selected_analyses(img_b64: str, view: str) -> None:
    ...
    st.session_state.guided_id_enabled = False
    st.session_state.guided_history = []
    st.session_state.guided_observations_text = ""
    st.session_state.guided_started = False
    st.session_state.guided_reveal_mode = False
...
def render_guided_id_prototype() -> None:
    single = st.session_state.single_result
    advanced = st.session_state.advanced_result
    image_b64 = st.session_state.current_image_b64

    if not image_b64 or (not single and not advanced):
        st.write("Run an analysis first to enable Guided ID.")
        return

    st.write("**Guided ID prototype**")
    st.caption("Developer-only test mode. The student must enter visible observations before the guide will help.")

    source_options = [label for label in ["Advanced counsel", "Single pass"] if (label == "Advanced counsel" and advanced) or (label == "Single pass" and single)]
    source_choice = st.radio(
        "Use hidden instructor context from",
        source_options,
        horizontal=True,
        index=0,
        key="guided_source_choice",
    )
    source_result = advanced if source_choice == "Advanced counsel" else single
    instructor_context = guided_context_summary(source_result, source_choice)

    c1, c2, c3 = st.columns([1.1, 1, 1])
    with c1:
        st.session_state.guided_hint_level = st.selectbox(
            "Hint level",
            ["Minimal", "Balanced", "Strong"],
            index=["Minimal", "Balanced", "Strong"].index(st.session_state.guided_hint_level),
            key="guided_hint_level_select",
        )
    with c2:
        st.session_state.guided_reveal_mode = st.checkbox(
            "Allow answer reveal",
            value=st.session_state.guided_reveal_mode,
            help="When enabled, the guide may provide the likely identification once asked.",
        )
    with c3:
        if st.button("Reset Guided ID", use_container_width=True):
            st.session_state.guided_history = []
            st.session_state.guided_observations_text = ""
            st.session_state.guided_started = False
            st.rerun()
    ...
    observations_text = st.text_area(
        ...
        value=st.session_state.guided_observations_text,
        height=120,
        placeholder="Enter at least 3 visible observations, one per line.",
        key="guided_observations_box",
    )
    st.session_state.guided_observations_text = observations_text
    observation_items = parse_student_observations(observations_text)
    st.caption(f"Observation count: {len(observation_items)}")

    if not st.session_state.guided_started:
        if st.button("Start Guided ID", use_container_width=False):
            if len(observation_items) < 3:
                st.warning("Enter at least 3 separate visible observations before starting Guided ID.")
            else:
                opening_message = "Initial student observations:\n" + "\n".join(f"- {item}" for item in observation_items)
                try:
                    answer = call_openai_guided_id(
                        student_message=opening_message,
                        image_b64=image_b64,
                        instructor_context=instructor_context,
                        chat_history=[],
                        hint_level=st.session_state.guided_hint_level,
                        reveal_mode=st.session_state.guided_reveal_mode,
                    )
                    st.session_state.guided_history = [
                    ...
                    st.error(f"Guided ID failed to start: {e}")
        return
    ...
    prompt = st.chat_input("Student reply for Guided ID", key="guided_chat_input")
    if prompt:
        st.session_state.guided_history.append({"role": "user", "content": prompt})
        with st.chat_message("user"):
            st.write(prompt)
        with st.chat_message("assistant"):
            with st.spinner("Guiding..."):
                try:
                    answer = call_openai_guided_id(
                        student_message=prompt,
                        image_b64=image_b64,
                        instructor_context=instructor_context,
                        chat_history=st.session_state.guided_history[:-1],
                        hint_level=st.session_state.guided_hint_level,
                        reveal_mode=st.session_state.guided_reveal_mode,
                    )
                except Exception as e:
                    answer = f"Guided ID failed: {e}"
            st.write(answer)
        st.session_state.guided_history.append({"role": "assistant", "content": answer})
...
def render_dev_tools() -> None:
    ...
    toggle_label = "Hide Guided ID prototype" if st.session_state.guided_id_enabled else "Launch Guided ID prototype"
    if st.button(toggle_label, use_container_width=True):
        st.session_state.guided_id_enabled = not st.session_state.guided_id_enabled
        st.rerun()

    if st.session_state.guided_id_enabled:
        with st.container(border=True):
            render_guided_id_prototype()
    ...
    with c2:
        export_payload = {
            ...
            "guided_id_enabled": st.session_state.guided_id_enabled,
            "guided_history": st.session_state.guided_history,
            "guided_hint_level": st.session_state.guided_hint_level,
            "guided_reveal_mode": st.session_state.guided_reveal_mode,
        }
        st.download_button(
